...
- `perso_photo.py` — fabrique la fiche d'un personnage d'après PHOTO, en deux temps :
  `portrait` (portraits serrés, avec lecture préalable de la photo par un modèle de
  vision) puis `fiche` (corps entier depuis le portrait retenu). `affiner` = rattrapage
  comparatif, à n'utiliser que sur une base ratée. `--style maison|pixar`, `--adulte`
